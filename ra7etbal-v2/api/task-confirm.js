...
async function handleEscalationAnswer(req, res) {
  ...
  try {
    ...
    // owner-WhatsApp-reply correlation path (api/_owner-escalation-reply.js).
 ...
 * reused by the owner-WhatsApp-reply correlation path
 * (api/_owner-escalation-reply.js) without a second, independently
...
export async function resolveAndDeliverEscalationAnswer({
  supabaseUrl, serviceKey, userId, deepLinkToken, escalation: existing, staffMessage, staffContextText, decision, instructionText,
}) {
  const headers = {
    apikey: serviceKey,
    Authorization: 'Bearer ' + serviceKey,
