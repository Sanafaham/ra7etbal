...
describe("canonical path source adapters", () => {
  ...
  it("keeps note/to-do conversion reminder paths on task creation plus reminder scheduling", () => {
    ...
    expect(inbox).toMatch(/async function handleRemindSubmit[\s\S]*createTask\(\{[\s\S]*type:\s*"reminder"[\s\S]*scheduleReminderPush\(/);
    expect(todos).toMatch(/async function handleRemindSubmit[\s\S]*createTask\(\{[\s\S]*type:\s*"reminder"[\s\S]*scheduleReminderPush\(/);
  });
