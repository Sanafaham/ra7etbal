...
describe("canonical path source adapters", () => {
  ...
  it("keeps voice direct tools wired to their canonical low-level helpers", () => {
    ...
    expect(widget).toMatch(/save_note:\s*\(params[^)]*\)\s*=>\s*\{[\s\S]*guardCurrentToolInvocation\("save_note",\s*params\)[\s\S]*runDirectToolWithDiagnostic\("save_note",\s*params,\s*\(\)\s*=>\s*saveNote\(params\)\)/);
    expect(widget).toMatch(/const saveNote = useCallback\([\s\S]*saveCarsonNote\(/);
    expect(widget).toMatch(/create_todo:\s*\(params[^)]*\)\s*=>\s*\{[\s\S]*guardCurrentToolInvocation\("create_todo",\s*params\)[\s\S]*runDirectToolWithDiagnostic\("create_todo",\s*params,\s*\(\)\s*=>\s*createTodoTool\(params\)\)/);
