...
export default function Confirm() {
  ...
  return (
    <section className="mx-auto max-w-md space-y-5 rounded-2xl border border-border bg-surface p-6 shadow-sm">
      ...
      {loadState === "loading" && (
        <div className="flex items-center justify-center py-6 text-ink">
      ...
      {loadState === "ready" && info && (
        <>
          <div className="space-y-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-ink">Task</p>
              <p className="mt-1 text-base leading-snug text-ink">{info.description}</p>
              {info.assignedTo && (
                <p className="mt-0.5 text-sm text-ink">For: {info.assignedTo}</p>
              )}
            </div>
                ...
                <div className={info.proofImageUrls.length === 1 ? "" : "grid grid-cols-2 gap-2"}>
                  {info.proofImageUrls.map((url, i) => (
                    <img
                      ...
                      className="aspect-square w-full rounded-xl border border-gold/35 object-cover shadow-sm"
                    />
                  ))}
                </div>
              </div>
          ...
          {info.status === "done" ? (
            ...
            <>
              ...
              <div className="space-y-2">
                <p className="text-xs font-medium uppercase tracking-wide text-ink">
                  Proof photo
                  <span className="ml-1 normal-case text-ink">
                    ...
                    {proofPhotos.map((photo, i) => (
                      <div key={photo.id} className="relative">
                        ...
                        <button
                          ...
                          className="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-cream text-ink shadow transition hover:bg-charcoal disabled:opacity-50"
                        >
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-gold/40 bg-surface-subtle px-4 py-3 text-sm text-ink transition hover:border-gold disabled:cursor-not-allowed disabled:opacity-50"
                >
                ...
                {proofLimitWarning && (
                  <p className="text-xs text-ink">{proofLimitWarning}</p>
                )}
                ...
                {proofError && (
                  <p className="text-xs text-ink">{proofError}</p>
                )}
              </div>
              ...
              <div className="space-y-1">
                <label
                  ...
                  className="text-xs font-medium uppercase tracking-wide text-ink"
                >
                  Note for the owner
                  <span className="ml-1 normal-case text-ink">(optional)</span>
                </label>
                <textarea
                  id="worker-reply"
                  ...
                  className="w-full rounded-xl border border-border bg-surface-subtle px-3 py-2 text-sm text-ink placeholder:text-ink focus:border-gold focus:outline-none disabled:opacity-50"
                />
              </div>
