...
export default function ElevenLabsAgentWidget({
  ...
  // Second Brain Slice 2 gate: off (undefined/unset) everywhere by
  // default, including Production Carson — set only on the isolated
  // non-production project while the isolated live gate is being run.
  // When on, every real conversation automatically carries a fresh
  // owner-authenticated Second Brain voice binding (see
  // startCarsonSession below); when off, this file's behavior is
  // byte-for-byte what it was before this gate existed. Fresh name, not
  // reused from the parked Stage2A/C-03 experiment.
  const secondBrainVoiceEnabled = import.meta.env.VITE_ENABLE_CARSON_SECOND_BRAIN_VOICE === "true";
  ...
  // 2026-08-25 production incident: ElevenLabs' event order between the
  // "speaking" mode change (audio) and "agent_message" (text, drives
  // onMessage -> setLastCarsonMessage) is NOT universal. A boolean
  // clear-once guard that only reacted to "speaking" wiped a real,
  // already-displayed tool-turn transcript when agent_message arrived
  // BEFORE speaking (proven in production for send_delegation/
  // create_reminder turns, eventIds 137 and 269 — agent_message logged,
  // then "speaking" 470-492ms later). reduceCarsonTranscriptTurn (see
  // carson-transcript-turn-state.ts) tracks displayed-vs-not explicitly
  // instead of inferring it from event order, so both orderings are safe:
  // once a real transcript has been displayed for this turn, no later
  // "speaking" transition (including the multi-sentence-TTS repeats
  // already documented at turnLatencyLoggedForEventIdRef's call site) can
  // ever blank it again. Reset alongside toolRanForCurrentTranscriptRef at
  ...
  const forceCleanupSession = useCallback(
    (teardownReason: string, options?: { showEndedMessage?: boolean; clearError?: boolean }) => {
      ...
      carsonTranscriptTurnStateRef.current = "pending";
  ...
  const startCarsonSession = useCallback(async (requestedChannel: CarsonChannel = "voice") => {
    ...
    try {
      ...
      // ─────────────────────────────────────────────────────────────────────
      // Second Brain Slice 2: automatic voice-boundary binding injection
      // (gated — see secondBrainVoiceEnabled above), fetched before the
      // connect timeout is armed so that timer only ever bounds the actual
      // SDK handshake, never our own pre-flight fetch. Reuses
      // issueSecondBrainVoiceBinding/getSupabaseAccessToken from
      // carson-second-brain-voice-binding.ts unchanged — the owner never
      // sees or handles this value. Fetched fresh for every conversation
      // start (the SDK sends customLlmExtraBody once, at session init,
      // same as dynamicVariables below — there is no supported way to
      // update it mid-conversation). Fails closed: if a fresh binding
      // can't be issued, the session is not started at all — this throws
      // into the existing catch block below, which already shows the
      // owner a safe error.
      let secondBrainCustomLlmExtraBody: { carson_second_brain_binding: string } | undefined;
      if (secondBrainVoiceEnabled) {
        const bindingResult = await issueSecondBrainVoiceBinding({ getAccessToken: getSecondBrainVoiceAccessToken });
        if (bindingResult.status !== "ready") {
          throw new Error("Second Brain voice binding unavailable");
        }
      ...
      const conv = await Conversation.startSession({
        ...
        onModeChange: ({ mode: m }) => {
          ...
          if (m === "speaking") {
            // Clear the stale previous-turn transcript on this turn's first
            // "speaking" transition ONLY — never if a real transcript for
            // THIS turn has already been displayed (see
            // carsonTranscriptTurnStateRef's declaration comment: agent_message
            // can arrive before OR after "speaking", proven in production).
            // Never replaced with placeholder/narration text: the real
            // transcript arrives through the existing, unmodified onMessage
            // -> setLastCarsonMessage(finalDisplayMessage) path.
            const speakingResult = reduceCarsonTranscriptTurn(carsonTranscriptTurnStateRef.current, {
              type: "speaking",
            });
            carsonTranscriptTurnStateRef.current = speakingResult.state;
            if (speakingResult.display !== undefined) {
              setLastCarsonMessage(speakingResult.display);
            }
            const active = activeExecuteLatencyRef.current;
        ...
        onMessage: ({ role, message, event_id }) => {
          ...
          if (role === "user") {
            ...
            // Real transcript for this turn has arrived — record it BEFORE
            // setLastCarsonMessage so a "speaking" transition racing in
  ...
  }, [agentId, secondBrainVoiceEnabled, authenticatedUserId, briefStateText, spokenBrief, displayName, mode, createReminder, createAutomation, sendDelegation, sendFollowup, saveCity, saveNote, actOnNote, executeInstruction, forceCleanupSession, endConversationSession, releaseMicWarmupStream, clearCarsonSessionTimers, clearPendingPhotoPreviews, onBeforeCallStart, runDirectToolWithDiagnostic, guardCurrentVoiceCapture, saveVoiceSessionSnapshot, ensureTypedHistoryLoaded]);
