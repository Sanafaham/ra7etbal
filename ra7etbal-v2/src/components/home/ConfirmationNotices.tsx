...
/** Owner-facing confirmation banners backed by tasks.dismissed_at. */
export default function ConfirmationNotices() {
  const { status } = useAuth();
  const location = useLocation();
  const tasks = useTasksStore(useShallow((state) => state.items));
  const dismiss = useTasksStore((state) => state.dismissConfirmationNotice);

  const visible = useMemo(() => selectConfirmationNotices(tasks), [tasks]);
    ...
    <div data-testid="confirmation-notices" className="mb-4 space-y-2.5" aria-live="polite">
      {visible.map((task) => {
        ...
        return (
          ...
          >
            <span aria-hidden className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-gold text-white">
            ...
            <button
              ...
              className="-mr-1 shrink-0 rounded-full p-1 text-ink transition hover:bg-gold/10 hover:text-gold"
            >
