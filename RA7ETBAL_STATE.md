...
**The following two limitations remain genuinely open engineering debt — NOT resolved by PR #209 or any prior round, both require a migration, explicitly out of scope for both PR #207 and #209:**
1. **Same-platform multi-device identity collision** — `navigator.platform` (e.g. `"iPhone"`) is not a true per-device identifier. Two genuinely different iPhones on the same account would collide, and the newer save would incorrectly disable the older device's still-valid subscription. No reliable device ID exists in the current schema without a migration (a new column). Reproducible by design, not yet observed in production.
2. **Non-atomic concurrent save/dedupe race** — the row write and the dedupe pass are two separate Supabase requests, not a transaction. Two saves for the same user+platform completing around each other could each disable the other's just-written row, leaving zero enabled rows for that device. A correct fix needs a single transactional Supabase RPC (upsert + stale-row disable in one statement) — a schema/migration change.

Do not mark either of these resolved without actually shipping the migration-based fix. Reopen as their own separately-scoped task if a reproduced regression from either is found.
