...
**Not independently verified**: the live ElevenLabs agent's current prompt/tool-schema state was not re-confirmed via direct API read-back in this task (no `ELEVENLABS_API_KEY` available in this environment); relying on the PR's own recorded hashes. A true authenticated end-to-end Carson retest (repeating the exact "Ask Christopher to reply..." utterance in production) has not yet been performed after the PR #106 fix — recommended before considering this fully closed.

Protect: the deterministic tool-policy gate's intent precedence and required-entity gating; the new `messageSendOutcomeRef`/`detectsUnconfirmedMessageSendClaim` truthfulness guard and its current-turn-only (not time-windowed) design. Do not reintroduce a shared time-windowed check for this class of fabrication guard. Reopen only on a reproduced production regression.
