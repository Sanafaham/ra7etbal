...
**Seventh confirmed production incident — false FAILURE claim on a genuinely successful send, during Stage 1's production verification sequence (PR #126, 2026-07-30, 03:03 Turkey time)**: Test 1 of the `route_people_action` verification plan ("Ask Christopher to reply yes if he can come tomorrow.") **succeeded** — Christopher received exactly one WhatsApp ("Operational update from Sana: Please reply yes if you can come tomorrow. Thank you.") — but Carson spoke and displayed "I wasn't able to send that. Please try again."

Traced via `carson_tool_diagnostics` for the 03:03 session before any code change: **not** a `route_people_action` bug — `legacy_people_tool_bypass` confirmed the model called `send_direct_whatsapp_message` directly this turn, so the new Stage 1 mapping code was never invoked at all. **Not** a mapping/serialization bug — the legacy handler ran normally (`invoked` → `handler_started` → `handler_success`) and the real WhatsApp send genuinely completed. **Not** a frontend timeout/fallback — zero `claim_overridden` diagnostic rows exist for this invocation, meaning the displayed text was the model's own, uncorrected original reply, never touched by the truthfulness override at all. Root cause: the existing truthfulness guard (`detectsUnconfirmedMessageSendClaim`, PR #106/#108/#118) only ever checked for a **false SUCCESS** claim ("Message sent to Christopher." when nothing actually sent) — it had no symmetric protection for the **opposite** case, a false FAILURE claim on a send that actually succeeded. "I wasn't able to send that. Please try again." never matched `MESSAGE_SEND_CONFIRMATION_PATTERN`, so the guard never engaged, and the model's premature/false-negative reply was spoken and displayed verbatim.

Fix (PR #126, merge commit `f3a5e26417c6031d706ba9be372981d134d1e514`, deployed to production `dpl_2GVKUgG7PMsVtvrZkdFHSgmcUjsw`, commit-matched, `www.ra7etbal.com`/`ra7etbal.com` aliased, `aliasError: null`): new `detectsUnconfirmedMessageSendFailureClaim` in `carson-direct-tool-override.ts` mirrors the existing success-claim check exactly, now wired into `resolveSanitizedCarsonDisplayMessage` — when the agent's reply claims a failure but a real `send_direct_whatsapp_message`/`send_delegation` call actually succeeded this turn, the displayed/spoken text is corrected to the tool's own true `resultText`, never the model's false claim. A genuinely accurate failure claim backed by a real tool failure is left untouched. The widget's deferred-await gate (which awaits the in-flight call's own settle before finalizing a truthfulness correction, PR #118) now uses the new `looksLikeMessageSendOutcomeClaim` — broadened to trigger on either a success-shaped or failure-shaped claim, not just success-shaped, so a premature claim of either kind waits for the real result instead of being spoken/displayed as-is.

Tests: 13 new behavioral tests in `carson-direct-tool-override.test.ts` (`detectsUnconfirmedMessageSendFailureClaim`, `looksLikeMessageSendOutcomeClaim`, and a PROTECTED regression test reproducing the exact confirmed incident verbatim — false failure claim + real success outcome → corrected to the tool's true result). Updated 2 pre-existing source-shape assertions in `ElevenLabsAgentWidget.direct-message-truthfulness.test.ts` and `ElevenLabsAgentWidget.typed-mode.test.ts` that held stale literal strings for the old gating condition/import line. Mutation-tested: disabling the new failure-claim wiring reproduces exactly one failing test (the new PROTECTED regression), nothing else; restoring passes. `TZ=UTC npm run test:carson-protected`: 998 passed, 4 skipped, 3 todo (49 files). Typecheck and production build both passed. The confirmed 2026-07-29 ~20:57 production success case and the duplicate-send guard (`isRecentDirectWhatsappDuplicate`/`recordDirectWhatsappSent`) are both untouched and remain protected by their existing tests.

No ElevenLabs prompt change was made or needed — this is a purely client-side truthfulness-guard gap, not a routing or wording defect. Per Sana's explicit instruction, Test 2 of the Stage 1 verification sequence was not started, and Sana was not asked to repeat Test 1 until this fix was confirmed and deployed. Protect: `detectsUnconfirmedMessageSendFailureClaim`'s symmetry with the existing success-claim check (do not let the two drift — any future wording fix to one should be evaluated against the other); `looksLikeMessageSendOutcomeClaim` triggering the deferred-await gate on either direction (do not narrow it back to success-only).

**Eighth confirmed production incident — same class as PR #126, escaped it via a different verb (PR #128, 2026-07-30, ~03:41 Turkey time)**: the immediate retest of the PR #126 fix ("Ask Christopher to reply yes if he can come tomorrow.", session `conv_6201kyr7eh6rfyvaymcbk3r5x3nv`) reproduced the same class of failure again — `send_direct_whatsapp_message` truly succeeded (`handler_started` → `handler_success` ~2.1s later, `recipient_person_id` confirmed, Christopher received the WhatsApp) — but Carson spoke/displayed **"I wasn't able to save that. Please say it again."** instead of the previous incident's "...able to send that." Zero `claim_overridden` for the turn, confirming this was the agent's own uncorrected reply, same signature as PR #126.

Confirmed root cause (traced via `carson_tool_diagnostics` before any code change, no speculation): PR #126's `FAILURE_CLAIM_PATTERN` hardcoded the verb to `send` in every alternative (`wasn't able to send`, `couldn't send`, `unable to send`, etc.) — "wasn't able to **save**" never matched. Neither the widget's deferred-await gate (`looksLikeMessageSendOutcomeClaim`) nor `resolveSanitizedCarsonDisplayMessage`'s own check ever engaged for this turn, so the model's premature/false-negative reply passed through untouched, identical in mechanism to PR #126 but with different surface wording. The model's own free-form apology phrasing varies by verb while meaning the same thing — PR #126 fixed the mechanism (the missing symmetric check existed) but had implicitly hardcoded a single word as if it were the only way the model would phrase a failure.

Fix (PR #128, merge commit `21c679ebcf2a3be073e2790dfe4410426cd46b04`, deployed to production `dpl_2RVBr6efSQkMG3v856hJoV1rS1Hu`, commit-matched, `www.ra7etbal.com`/`ra7etbal.com` aliased, `aliasError: null`): generalized the verb slot in every `FAILURE_CLAIM_PATTERN` alternative from a hardcoded `send` to any single word (`[a-z]+`), so `save` and any other verb variant of the same six apology frames (`wasn't able to X` / `couldn't X` / `unable to X` / `failed to X` / `didn't X` / `not able to X`) are now covered without hardcoding another word. No architecture change, no prompt change.

Tests: new PROTECTED regression test reproducing this exact incident verbatim (`"I wasn't able to save that. Please say it again."` + a confirmed success outcome → corrected to the tool's true result); added `"save"` to the existing verb-variant `it.each` coverage; updated one `looksLikeMessageSendOutcomeClaim` test whose expectation flipped correctly now that "couldn't get that sent" is recognized as failure-shaped (this only affects whether the client awaits the real outcome before finalizing display text — it does not by itself rewrite anything; `resolveSanitizedCarsonDisplayMessage` still only corrects when the tool's own outcome is a confirmed `"success"`). Mutation-tested: reverting to the hardcoded `send`-only regex reproduces exactly the 3 new/updated tests failing, nothing else; restoring passes. `TZ=UTC npm run test:carson-protected`: 998 passed, 49 files. Typecheck and build both passed.

**Honest residual risk, not fully closed by this fix**: the underlying failure mode is that the model's own free-form apology wording is not enumerable — a regex generalizing the verb slot covers any single-word variant of the six known apology frames, but a **third** occurrence using a structurally different phrasing (a frame not among those six, or a multi-word verb phrase) could still escape this check the same way "save" escaped PR #126. This is a fundamentally different class of problem than a fixed set of hardcoded words — no regex enumerates infinite free-form phrasing. If a third occurrence happens with novel phrasing, the fix is the same kind of targeted pattern-widening as this one, not a redesign. Do not claim this is now permanently closed; treat any further occurrence as evidence to widen the same patterns, not as a signal to redesign the mechanism (which is confirmed correct — the gap is coverage, not architecture).
