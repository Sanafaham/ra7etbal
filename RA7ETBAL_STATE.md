...
### Carson production canary least-privilege hardening — IMPLEMENTED, PENDING PROTECTED MERGE
...
Real-Postgres forward/rollback/reapply verification covers exact grants, owner attributes, safe configuration, healthy output, both invariant-breaking counterfactuals, wrong-token rejection, anon mutation rejection, bounded output, and rollback removal. Production configuration remains deliberately unset until the protected PR merges and the migration is deployed; Sana must generate/configure the token outside chat afterward. No production business runtime, Vercel route, WhatsApp behavior, reminder/automation semantics, Notifications Inbox behavior, or raw production data changed.
