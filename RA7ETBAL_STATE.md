...
### Push Subscription Installation Identity + Atomic Replacement — FIXED, MERGED, DEPLOYED, PRODUCTION DATABASE VERIFIED; live-device re-test PENDING

Status: implementation complete, all CI + ephemeral-Postgres verification passing, merged to `main`, deployed to production (app code + migration both live). The one remaining item is a live on-device rotation/stress re-test on Sana's actual iPhone, matching the PR #207 protocol below — that needs Sana's interactive participation and has not yet been performed.
...
**Still pending — do not claim this fully verified until done:** repeat the PR #207 live-device protocol on Sana's actual iPhone — rotate/refresh the subscription, rapid repeated save/refresh stress test, confirm exactly one enabled row for that installation, confirm the Mac/other installations remain untouched, confirm a real reminder still arrives. Requires Sana's interactive participation.

**Follow-up, explicitly deferred, NOT YET STARTED — do not begin until the live-device re-test above is done:** "Push Subscription Installation Management / Orphan Resolution" — a safe, positive-evidence way for Sana to identify and disable obsolete installations (rows orphaned by iOS eviction/reinstall with no safe automatic signal) without any automatic guessing. Scoped as a user-facing management surface (e.g. in Settings), not a background sweep — a background age-based sweep was explicitly rejected during this task's design review as unsafe (see architecture notes above).
