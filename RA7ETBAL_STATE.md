...
### Owner completion push reliability — REOPENED: DEVICE-STAGE OBSERVABILITY DEFECT CONFIRMED (PR #240)
...
**Current status: MERGED. DEPLOYED. PROVIDER-SIDE PRODUCTION VERIFIED. DEVICE-STAGE OBSERVABILITY DEFECT OPEN.** Do not mark this COMPLETED/PRODUCTION VERIFIED/PROTECTED again until the `dueAt`/`confirmed_at` binding is fixed and a fresh production completion event shows a successful (2xx) `notification-receipt` round trip with real `service_worker_received`/`show_notification_attempted` rows. Record explicitly, for anyone reading this section: the owner did physically receive the push (confirmed twice, independently, by direct observation); provider-side durable evidence (`provider_send_attempted`/`provider_accepted`, endpoint dedup, correct `kind`, retention) remains fully proven and does not need to be reverified; the gap is specifically that the system cannot yet truthfully record which device-side stages the notification reached, because its own validation gate incorrectly rejects genuine device receipts.

**Proposed narrow fix (read-only investigation only, not yet implemented — do not implement until this is explicitly approved):** thread the exact, already-persisted `tasks.confirmed_at` value into `sendOwnerPush()`'s receipt construction (e.g. accept a `confirmedAt` parameter from `task-confirm.js`'s call site, which already has `now`/`task.confirmed_at` in hand, and use it verbatim as the receipt's `dueAt` instead of a freshly-generated `sentAt`) so the completion receipt binds to the canonical persisted timestamp instead of a second, independently-generated one. Must preserve: reminder-receipt behavior and its `due_at` validation path (byte-for-byte unchanged — this only touches the `taskId`-gated completion branch), HMAC signing/verification (`signReminderReceipt`/`verifyReminderReceipt`, unchanged), the existing `kind`-based event separation, and requires no schema change (`reminder_delivery_events` already stores whatever `dueAt` it's given). Scope is one value's origin inside `sendOwnerPush()` plus threading it from `task-confirm.js`'s existing `now`/`confirmed_at` — not a new mechanism.
