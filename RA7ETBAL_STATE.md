...
### Corrected Path C — Stage 2A owner-authenticated Custom LLM boundary (2026-08-20) — ACCEPTANCE PASSED, PROTECTED

Stage 2A proves the division of labour that the whole of Corrected Path C depends on: **ElevenLabs owns ears, turn-taking and voice; Ra7etBal owns the brain, the tools and the execution truth.** A signed-in owner speaks on the isolated boundary-proof page, ElevenLabs transcribes and posts an OpenAI-compatible Chat Completions request to a Ra7etBal Custom LLM endpoint, and Ra7etBal returns one fixed, server-owned sentence — `Boundary proof successful.` — as SSE, which ElevenLabs speaks verbatim. No second model runs after the canonical response, and there is no tool, database, or execution surface anywhere in the path.

**Acceptance evidence.** Conversation `conv_7501m0gsp7v2e6mam5kbyrsrr405` (2026-08-20T23:56:51Z): real proof page, real signed-in owner, real voice, four owner turns. **Three** agent responses were exactly `Boundary proof successful.`; the **fourth** carried the same canonical sentence with a trailing ellipsis. That distinction is deliberate and must be preserved — this is not "four exact responses."

**Three real ElevenLabs-side defects were found and fixed** to reach acceptance: a missing `opening_line` dynamic variable (the agent's `first_message` interpolates it and the session cannot start without it), a disabled `custom_llm_extra_body` override in the agent's Security settings (which silently dropped the owner binding), and the `/chat/completions` routing gap (ElevenLabs treats the configured Server URL as a **base** URL and appends `/chat/completions` itself, so a single additive `vercel.json` rewrite was required).

**Security shape.** The endpoint fails closed at two independent gates: a dedicated provider secret (absent → 503, wrong → 401) and a server-issued, account-bound, HMAC-signed, 10-minute session binding minted only after the owner's Supabase JWT is verified (absent, expired, or foreign-signed → 401). Neither gate was weakened to make any test or dashboard check pass. Secrets live only in the isolated Vercel project and the ElevenLabs workspace secret store; the ElevenLabs store is write-only, so a provider secret can be re-set on both sides but never read back and compared.

**Protection (this closure).** `api/carson-custom-llm-stage2a.contract.test.js` pins the ElevenLabs wire contract (the full documented request body, OpenAI-compatible chunk shape, a single `[DONE]`, no tool surface, fail-closed before any streaming). `api/golden-journey-stage2a-owner-boundary.test.js` proves the whole owner journey against the real handler through an injected Supabase `authenticate` seam. Both are in `test:carson-protected`. `scripts/carson-production-canary.mjs` gained a read-only, credential-free probe (`evaluateStage2aBoundaryFailClosed`) that sends one unauthenticated POST and requires a 401 carrying the handler's own `Unauthorized provider` body; because Stage 2A lives on a separate non-production deployment, it runs only when `STAGE2A_BOUNDARY_URL` is supplied and is otherwise reported under the canary report's `notRun` list — never counted as a pass. The registry capability `carson_stage2a_custom_llm_boundary_proof` is now `PROTECTED`.

**What is still NOT proven.** No offline test and no canary can prove that ElevenLabs actually dispatches an outbound request to the configured URL — for a long period it attempted generation and aborted before any HTTP request left its infrastructure. Only the live human acceptance above evidences that half of the boundary, and it should be re-run when the ElevenLabs agent or the proof page changes, never per CI run. The historical ElevenLabs `custom_llm_error` 1002 root cause remains **CANNOT VERIFY**: the original secret values are unrecoverable, so it must not be attributed to credentials, tools, MCP, Vercel Deployment Protection, routing, or any other component as a proven cause.

**Out of scope and unchanged.** Stage 2B is not started. Production Carson, staging Carson, WhatsApp, Calendar, reminders, automations, Supabase, and `main` were not touched. Tool/MCP/Perplexity/Tavily restoration on the Stage 2A agent, Tavily credential rotation, and the production Carson duplicate tool names (`search_calendar_history` ×2, `get_commitment_history` ×2) all remain open and separate.
