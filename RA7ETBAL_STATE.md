...
Also drafted (not auto-applied, per this directory's own README policy) `docs/elevenlabs-prompt-patches/2026-07-29-direct-message-truthfulness-and-routing.md` — additive prompt guidance reinforcing reply/respond routing and never claiming success without tool confirmation. Awaiting Sana's manual review/paste; not yet reflected in the live agent.

**Still open, not fixed here — a separate, smaller item**: no server-side diagnostic currently distinguishes "model never called a tool" vs. "gate rejected a real tool call" vs. "handler ran and exited early" vs. "backend request started and failed" — `guardCurrentToolInvocation` already records a `tool_policy_rejected` diagnostic with the exact reason, but only to the device's local `localStorage`, never to Supabase. Would need a new server-side diagnostics feature to close; out of scope for this fix.
