...
**Still open, not yet re-verified**: a third live retest of the exact same utterance has not yet been performed after PR #108. Deeper and explicitly out of this fix's scope: *why* `send_direct_whatsapp_message` keeps resolving in ~0ms with no real transport call for this exact phrasing at all — this truthfulness guard only guarantees Carson can no longer **claim** success without one; it does not make the real send happen. That may be a separate, upstream tool-selection/policy-routing question worth its own investigation if it recurs. Also still open: the live ElevenLabs agent's current prompt/tool-schema state was not independently re-confirmed via direct API read-back in this task (no `ELEVENLABS_API_KEY` available in this environment).

Protect: the deterministic tool-policy gate's intent precedence and required-entity gating; the `messageSendOutcomeRef`/`detectsUnconfirmedMessageSendClaim` truthfulness guard, its current-turn-only (not time-windowed) design, and its claim-pattern coverage (do not narrow it back to only the first incident's exact wording). Do not reintroduce a shared time-windowed check for this class of fabrication guard. Reopen only on a reproduced production regression.
