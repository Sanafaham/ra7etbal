...
**REQUIRES SANA (not closed, cannot be):** `hosting_operations`' Tier classification (an explicit product decision); the 4 GitHub Actions secrets Phase 7's canary and Phase 8's automated ledger updates need (`VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`); the `whatsapp_deliveries` single-writer RPC gatekeeper (its own design review).
