...
**Definition of Done met:** new `automation_delegation`/`automation_message` deliveries preserve canonical person identity (proved live, not just backfilled); Communication History retrieves them without a synthetic `messages` row; identity contradictions fail closed (tested); historical affected rows safely repaired (6/6, idempotent); focused/protected tests pass; production verification passes; this file updated.
