...
**Tenant isolation — reported honestly, not claimed as a full PASS.** Verified as a static/composition property: `fetchAttentionEvidence()` takes no caller-suppliable identity parameter; the only identity source is `supabase.auth.getUser()`, same pattern as its two production-proven siblings; new regression tests assert this. Relies on the same RLS policies on `tasks`/`staff_messages`/`automation_runs` already in production for those siblings. No new live cross-account (User A cannot see User B's rows) Postgres exercise was performed for this specific composition — the available automated test environment has no safe multi-account fixture for this read path, and building one was judged out of this slice's authorized scope (no schema/migration, no synthetic production accounts). Reported as a named, open limitation.
