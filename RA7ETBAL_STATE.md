...
**Next up (separate task, not started):** Canonical Staff Identity Cleanup — duplicate `Christopher`/`Ghulam`/`Nasira` "Test staff" `people` rows (created in a ~7-second batch on 2026-07-13) make the real staff rows ambiguous to name-based recipient resolution (`recipient_not_unique`). Discovered during Workstream 3 fix verification; read-only investigated, not yet touched.
