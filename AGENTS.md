...
Type and Talk must remain behaviorally aligned. They may use different transport adapters (a typed regex fast path vs. an ElevenLabs voice tool call), but both must converge on the same shared decision points (e.g. `sendDelegation()` in `ElevenLabsAgentWidget.tsx`, called by both Talk to Carson's `send_delegation` clientTool and Type to Carson's delegation fast path) so they cannot diverge. **Exception (2026-07-25 product decision, "Type to Carson is advisory-only" in `RA7ETBAL_STATE.md`):** Type to Carson is deliberately no longer permitted to execute any state-changing action, including delegation. Its delegation-shaped-request detection reuses the exact same pure parser `sendDelegation()`/`executeDelegationFastPath()` use internally (`parseDelegationFastPath`) so it cannot silently diverge on *what counts as* a delegation, but it never calls `sendDelegation()` itself — it returns an advisory message instead. This is an intentional, single, product-decision-gated exception (`TYPED_MODE_IS_ADVISORY_ONLY`), not a new per-channel divergence to be extended elsewhere.
